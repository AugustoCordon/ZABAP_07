CLASS zcl_abap_sql_07 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_abap_sql_07 IMPLEMENTATION.
...
SELECT SINGLE
  ...
  WHERE c~carrier_id    = 'LH'
    AND c~connection_id = '0400'
  INTO @DATA(connection_join_1).

    out->write(  `------------------------------------------` ).
    out->write(  `Example 7: Join of Connection and Airports (alternativa)` ).
    out->write( connection_join_1 ).
